...
export const CustomNode = memo(({ id, data, isConnectable = true }: NodeProps<NodeData>) => {
      ...
      <Tooltip>
        ...
        <TooltipContent>
          <div className="p-2">
            ...
            {data.username && <p className="text-sm">用户: {data.username}</p>}
            {data.amount && <p className="text-sm">金额: ¥{data.amount.toLocaleString()}</p>}
            {data.date && <p className="text-sm">日期: {data.date}</p>}
            {data.type && <p className="text-sm">类型: {data.type}</p>}
            {data.description && <p className="text-sm">描述: {data.description}</p>}
          </div>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
